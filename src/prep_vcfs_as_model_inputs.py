...
def parse_arguments():
    ...
    parser.add_argument("--save_dir", default="../../pnet_germline/processed/", help="Directory storing model-ready input")
...
def main():
    ...
    if WANDB_GROUP != "":
        SAVE_DIR = os.path.join(SAVE_DIR, f"wandb-group-{WANDB_GROUP}/converted-IDs-to-{CONVERT_IDS_TO}_imputed-germline_{ZERO_IMPUTE_GERMLINE}_imputed-somatic_{ZERO_IMPUTE_SOMATIC}_paired-samples-{USE_ONLY_PAIRED}/wandb-run-id-{wandb_run_id}")
        ...
        SAVE_DIR = os.path.join(SAVE_DIR, f"converted-IDs-to-{CONVERT_IDS_TO}_imputed-germline-{ZERO_IMPUTE_GERMLINE}_imputed-somatic-{ZERO_IMPUTE_SOMATIC}_paired-samples-{USE_ONLY_PAIRED}/wandb-run-id-{wandb_run_id}")
    ...
    # TODO: update paths
    germline_rare_lof_f = os.path.join(GERMLINE_DATADIR, "prostate/prostate_germline_vcf_subset_to_germline_tier_12_and_somatic_passed-universal-filters_rare_high-impact.txt")
    germline_rare_missense_f = os.path.join(GERMLINE_DATADIR, "prostate/prostate_germline_vcf_subset_to_germline_tier_12_and_somatic_passed-universal-filters_rare_moderate-impact.txt")
    germline_common_lof_f = os.path.join(GERMLINE_DATADIR, "prostate/prostate_germline_vcf_subset_to_germline_tier_12_and_somatic_passed-universal-filters_common_high-impact.txt")
    germline_common_missense_f = os.path.join(GERMLINE_DATADIR, "prostate/prostate_germline_vcf_subset_to_germline_tier_12_and_somatic_passed-universal-filters_common_moderate-impact.txt")
    ...
    if USE_ONLY_PAIRED: # want to run stuff if paired; note that this will only work correctly if the IDs are correctly harmonized
        ...
        somatic_mut, somatic_amp, somatic_del, germline_rare_lof, germline_rare_missense, germline_common_lof, germline_common_missense, additional, y = data_manipulation.restrict_to_overlapping_indices(
    ...
    somatic_mut, somatic_amp, somatic_del = prostate_data_loaders.zero_impute_somatic_datasets(germline_datasets=[germline_rare_lof, germline_rare_missense, germline_common_lof, germline_common_missense], 
                                 somatic_datasets=[somatic_mut, somatic_amp, somatic_del], 
                                 zero_impute_somatic=ZERO_IMPUTE_SOMATIC)
    ...
    somatic_mut, somatic_amp, somatic_del, germline_rare_lof, germline_rare_missense, germline_common_lof, germline_common_missense = prostate_data_loaders.restrict_to_genes_in_common(somatic_mut, somatic_amp, somatic_del, 
                                                                                                                                                                                        germline_rare_lof, germline_rare_missense, germline_common_lof, germline_common_missense)    
