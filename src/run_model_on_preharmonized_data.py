...
def main():
    ...
    logging.info("Loading data from directory {}".format(config['input_data_dir']))
    input_data_wandb_id = config.get('input_data_wandb_id', '')

    for name, info in config['genetic_data'].items():
        config['genetic_data'][name]['df'] = pd.read_csv(os.path.join(config["input_data_dir"], info['filename']))

    config['confounder_data']['df'] = pd.read_csv(os.path.join(config["input_data_dir"], config['confounder_data']['filename']))
    config['target']['df'] = pd.read_csv(os.path.join(config["input_data_dir"], config['target']['filename']))
