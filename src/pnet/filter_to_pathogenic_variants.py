...
def make_confliciting_evidence_table(
    vep_df, id_col="Uploaded_variation", clinsig_col=None, conflicting_col=None, clin_conflict=None, conseq_col=None
):
    if clinsig_col is None:
        clinsig_col = "ClinVar_updated_2022Aug_CLNSIG"
...
def variant_selection_workflow(
    ...
    if len(vep_conflicting) > 0:
        ...
        vep_conflicting_table = make_confliciting_evidence_table(
            vep_conflicting, id_col="Uploaded_variation", clinsig_col=clinsig_col, clin_conflict=clin_conflict
        )
        ...
        vep_all_patho_variants = pd.concat(
            [vep_truncating_nob_lof, vep_patho, vep_conflicting_table_include_merged], ignore_index=True
        )
