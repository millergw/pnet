...
def load_tcga_dataset(directory_path, load_mut=False, rna_standardized=True):
    if rna_standardized:
        rna = (
            pd.read_csv(
                directory_path + "/data_mrna_seq_v2_rsem_zscores_ref_all_samples.txt", sep="\t", low_memory=False
            )
            .dropna()
            .set_index("Hugo_Symbol")
            .drop(["Entrez_Gene_Id"], axis=1)
            .T
        )
    else:
        rna = (
            pd.read_csv(directory_path + "/data_mrna_seq_v2_rsem.txt", sep="\t", low_memory=False)
            .dropna()
            .set_index("Hugo_Symbol")
            .drop(["Entrez_Gene_Id"], axis=1)
            .T
    ...
    cna = (
        pd.read_csv(directory_path + "/data_cna.txt", low_memory=False, sep="\t")
        .dropna()
        .set_index("Hugo_Symbol")
        .drop(["Entrez_Gene_Id"], axis=1)
        .T
    ...
    tumor_type = pd.DataFrame(
        len(indices) * [directory_path.split("/")[-1].split("_")[0]], index=indices, columns=["tumor"]
    )

    if load_mut:
        muts = pd.read_csv(directory_path + "/data_mutations.txt", delimiter="\t")
        grouped_muts = (
            muts[muts["Variant_Classification"].apply(lambda x: MUTATIONS_DICT[x]) != "Silent"][
...
def get_auc_prc(pred_proba, target):
    ...
    if len(target.shape) > 1 and target.shape[1] > 1:
        ...
        auc_prc = torchmetrics.functional.average_precision(
            pred_proba, target, task="multiclass", num_classes=num_classes
        )
    else:
