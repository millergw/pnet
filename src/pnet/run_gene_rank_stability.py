...
prostate_cnv = pd.read_csv(
    os.path.join(DATADIR, "pnet_database/prostate/processed/P1000_data_CNA_paper.csv")
)
...
prostate_response = pd.read_csv(
    os.path.join(DATADIR, "pnet_database/prostate/processed/response_paper.csv")
)
...
prostate_genes = list(
    set(prostate_genes["genes"])
    .intersection(set(prostate_mutations.columns))
    .intersection(set(prostate_cnv.columns))
)
...
prostate_mutations = prostate_mutations[
    list(set(prostate_mutations.columns).intersection(prostate_genes))
].copy()
...
for r in range(20):
    ...
    if MODEL_TYPE == "pnet":
        ...
        report_and_eval.savefig(
            plt, os.path.join(SAVEDIR, f"deviance_per_boosting_iteration_run_{r}")
        )
        wandb.log({"convergence plot": plt})
        plt.show()
