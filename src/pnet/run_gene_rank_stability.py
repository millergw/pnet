...
import wandb
from pnet import model_selection, pnet_loader, report_and_eval
...
logger.info("Setting data and save directories")
# DATADIR = '/mnt/disks/pancan' # Marc's
DATADIR = "../../pnet_germline/data"  # Gwen's
SAVEDIR = f"../../pnet/results/somatic_{MODEL_TYPE}_eval_set_{EVAL_SET}_different_machine"  # Gwen's
report_and_eval.make_dir_if_needed(SAVEDIR)

logger.info("Loading data")
prostate_mutations = pd.read_csv(
    os.path.join(
        DATADIR,
        "pnet_database/prostate/processed/P1000_final_analysis_set_cross_important_only.csv",
    )
)
...
prostate_genes = pd.read_csv(
    os.path.join(DATADIR, "pnet_database/genes/tcga_prostate_expressed_genes_and_cancer_genes.csv")
)
prostate_genes = list(
    set(prostate_genes["genes"]).intersection(set(prostate_mutations.columns)).intersection(set(prostate_cnv.columns))
)
...
genetic_data = {
    "somatic_mut": prostate_mutations,
    "somatic_amp": prostate_amp,
    "somatic_del": prostate_del,
}
logger.info(f"We are using the datasets: {list(genetic_data.keys())}")

logger.info("Defining train/test indices")
...
train_dataset, test_dataset = pnet_loader.generate_train_test(
    genetic_data,
    target=prostate_response,
    train_inds=train_inds,
    test_inds=test_inds,
    gene_set=prostate_genes,
)
...
for r in range(20):
    wandb.init(
        ...
        group=f"{MODEL_TYPE}_stability_experiment_006",
    )
    wandb.config.update(
        {
    ...
    if MODEL_TYPE == "pnet":
        (
            ...
            gene_imp,
            layerwise_imp,
        ) = model_selection.run_pnet(genetic_data, prostate_response, train_inds, test_inds)
        aucs.append(auc)
        ...
        logger.info(
            f"Getting the model predictions, performance metrics, feature importances (as appropriate), and save the results to {SAVEDIR}."
        )
        ...
        report_and_eval.evaluate_interpret_save(
            ...
            who=EVAL_SET,
            save_dir=SAVEDIR,
        )  # TODO: who=val/test is hardcoded

        logger.info(f"Making loss plots to check convergence for model {r}")
        plt = report_and_eval.get_loss_plot(
            train_losses=train_scores, test_losses=test_scores, test_label=f"{EVAL_SET} loss"
        )
        ...
        logger.info(f"Making deviance plots to check convergence/overfitting for model {r}")
        ...
        plt = report_and_eval.get_loss_plot(
            ...
            test_label=f"{EVAL_SET} deviance",
    ...
    if MODEL_TYPE in ["rf", "bdt"]:
        ...
        gene_imp = report_and_eval.evaluate_interpret_save(
            model=model,
            pnet_dataset=train_dataset,
            model_type=MODEL_TYPE,
            who="train",
...
logger.info(f"Saving gene importances to file in {SAVEDIR}")
# Save gene_imps to a Pickle file
with open(os.path.join(SAVEDIR, "gene_imps.pkl"), "wb") as file:
    pickle.dump(gene_imps, file)

if MODEL_TYPE == "pnet":
    logger.info(f"Saving layer importances and AUCs to files in {SAVEDIR}")
