...
def binarize_burden_mat(value):  # TODO: check if these are equivalent
    return data_manipulation.binarize(value, set_as_zero=0.0)
    # if value != 0.:
    #     return 1
    # else:
    #     return 0
...
def n_variants_per_sample_from_vcf(
    vcf, savefig_f=False, plot_title="# variants per sample", plot_id=None, plot_xlabel="# variants/sample"
):
    """
    Inputs:
...
def filter_annotated_vcf_by_gene_list(
    annot_vcf, annot_vcf_f, gene_list, save_filtered_df_path="annotated_vcf_filtered_to_genes_of_interest.txt"
):
    """
...
def filter_annotated_vcf_by_gene_list_chunking(
    annot_vcf_f, gene_list, save_filtered_df_path="annotated_vcf_filtered_to_genes_of_interest.txt", chunksize=20000
):
    """
