...
from pnet import Pnet, pnet_loader


def main():
    prostate_mutations = pd.read_csv(
        "/mnt/disks/pancan/pnet_database/prostate/processed/P1000_final_analysis_set_cross_important_only.csv"
    )
    prostate_mutations.set_index("Tumor_Sample_Barcode", inplace=True)

    prostate_cnv = pd.read_csv(
        "/mnt/disks/pancan/pnet_database/prostate/processed/P1000_data_CNA_paper.csv"
    )
    ...
    prostate_response = pd.read_csv(
        "/mnt/disks/pancan/pnet_database/prostate/processed/response_paper.csv"
    )
    ...
    prostate_genes = pd.read_csv(
        "/mnt/disks/pancan/pnet_database/genes/tcga_prostate_expressed_genes_and_cancer_genes.csv"
    )
    prostate_genes = list(
    ...
    for method_name in regularization_method_values:
        for alpha in h1_regularization_strength:
            save_path = "../results/hyperparam_search/h1regularization-{}_alpha-{}".format(
                method_name, alpha
            )
            ...
            train_loader, test_loader = pnet_loader.to_dataloader(
                train_dataset, test_dataset, batch_size
            )
            model, train_scores, test_scores = Pnet.train(
                model,
                train_loader,
                test_loader,
                save_path + "/model",
            ...
            df = pd.DataFrame(
                index=["train_loss", "test_loss"], data=[train_scores, test_scores]
            ).transpose()
