...
def calculate_probabilities(df):
    ...
    """
    logger.info("Calculating gene-level probabilities...")
...
def plot_probabilities(probabilities, title="Gene Mutation Probabilities"):
    ...
    """
    logger.info("Plotting probabilities: %s", title)
...
def plot_probability_histogram(probabilities, bins=10, title="Histogram of Gene Mutation Probabilities"):
    ...
    Parameters:
        ...
        title (str): Plot title.
    """
    logger.info("Plotting histogram: %s", title)
    plt.figure(figsize=(10, 6))
...
def plot_probability_dotplot(probabilities, title="Dot Plot of Gene Mutation Probabilities"):
    ...
    Parameters:
        ...
        title (str): Plot title.
    """
    logger.info("Plotting dot plot: %s", title)
    plt.figure(figsize=(10, 6))
...
def generate_perturbed_data(df, perturb_type="shift", k=3, sample_fraction=0.5, gene_fraction=0.5):
    ...
    """
    logger.info("Starting perturbation process...")
    np.random.seed(42)  # For reproducibility

    # Calculate original probabilities
    logger.info("Calculating original probabilities...")
    ...
    logger.info("Perturbing %d genes and %d samples...", num_genes, num_samples)
    ...
    logger.info("Selected genes for perturbation:\n%s", perturbed_genes)
    logger.info("Selected samples for perturbation:\n%s", perturbed_samples)

    # Perturb probabilities
    perturbed_probabilities = gene_probabilities.copy()
    if perturb_type == "shift":
        logger.info("Applying 'shift' perturbation...")
        ...
        logger.info("Applying 'scale' perturbation...")
        ...
        logger.info("Applying 'constant_offset' perturbation...")
        perturbed_probabilities[perturbed_genes] = np.minimum(gene_probabilities[perturbed_genes] + k, 1)

    logger.info("Perturbed probabilities:\n%s", perturbed_probabilities.head())
    ...
    logger.info("Perturbed dataset generated.")
    return perturbed_df
