...
class ReactomeNetwork:
    def __init__(self, gene_list, embedding_length=0, use_embeddings=False, trim=0, max_depth=6, pathways_to_drop=[]):
        ...
        embedding_length : int, optional
            Length of the gene embedding vector (default is 0).
        use_embeddings : bool, optional
            Whether to include embeddings in the input mask (default is False).
        trim : int, optional
            Number of nodes to trim at each level (default is 0).
        max_depth : int, optional
            Maximum depth of the pathway hierarchy (default is 6).
        pathways_to_drop : list of str, optional
            List of pathways to exclude from the network (default is an empty list).
        """
    ...
    def get_masks(self, nbr_genetic_input_types, regulatory=False):
        """
        Transforms pd.DataFrame adjacency matrices into binary np.array masks.
    ...
    def get_masks_when_making_input_mask_in_pnet_loader(self, regulatory=False):
        """
        Transforms pd.DataFrame adjacency matrices into binary np.array masks.
        :param nbr_genetic_input_types: int; number of genetic input modalities provided
        :return: (list(np.array), list(np.array), np.array); a list of adjacency matrices per layer and a list of
        ...
        if regulatory:
            reg_mask = self.get_reg_mask()
            return gene_masks, pathway_masks, reg_mask.values
        else:
